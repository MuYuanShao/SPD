-- Core schema: departments, users, roles, permissions, products, inventory, purchasing, quota packages, etc.
...
CREATE TABLE IF NOT EXISTS `purchase_demand` (
  ...
  UNIQUE KEY `uk_purchase_demand_no` (`demand_no`),
...
CREATE TABLE IF NOT EXISTS `quota_package_event` (
  ...
  `remark` VARCHAR(500) DEFAULT NULL COMMENT '备注',
  `event_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`event_id`),
