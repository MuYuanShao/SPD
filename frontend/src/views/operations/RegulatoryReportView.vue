...
const configs = {
  ...
  inventory: { title: '全院物资进销存汇总表', code: 'inventory-movement-summary', note: '统计口径：依据不可变库存流水回溯期初、期间收发及期末库存。' },
} as const
    ...
    <section class="reconciliation-table-card" :aria-busy="loading">
      <div class="reconciliation-table-scroll">
        <table v-if="reportType === 'supplier'" class="reconciliation-table supplier-ledger-table">
          ...
          <tbody><tr v-if="loading"><td colspan="17" class="table-message">正在加载供货明细…</td></tr><tr v-else-if="!rows.length"><td colspan="17" class="table-message">当前筛选条件下暂无供货数据</td></tr><tr v-for="(item, index) in supplierRows" v-else :key="`${item.orderNo}-${item.productCode}-${index}`"><td>{{ (query.page - 1) * query.size + index + 1 }}</td><td>{{ item.supplyDate }}</td><td>{{ item.supplierName }}</td><td>{{ item.manufacturerName }}</td><td class="code-cell">{{ item.productCode }}</td><td class="code-cell">{{ item.medicalInsuranceCode }}</td><td class="product-cell">{{ item.productName }}</td><td>{{ item.specModel }}</td><td>{{ item.registrationNo }}</td><td class="numeric">¥{{ amount(item.unitPrice) }}</td><td class="numeric">{{ quantity(item.supplyQuantity) }}</td><td class="numeric">¥{{ amount(item.supplyAmount) }}</td><td class="code-cell">{{ item.orderNo }}</td><td>{{ item.contractNo }}</td><td>{{ item.acceptanceDepartment }}</td><td>{{ item.acceptanceUser }}</td><td>{{ item.acceptanceTime }}</td></tr></tbody>
        </table>
        <table v-else-if="reportType === 'centralized'" class="reconciliation-table centralized-table">
          ...
          <tbody><tr v-if="loading"><td colspan="12" class="table-message">正在计算集采执行进度…</td></tr><tr v-else-if="!rows.length"><td colspan="12" class="table-message">暂无集采任务或中选物资</td></tr><tr v-for="item in centralizedRows" v-else :key="`${item.batchCode}-${item.productCode}`"><td><strong>{{ item.batchName }}</strong><small>{{ item.batchCode }}</small></td><td class="product-cell">{{ item.productName }}</td><td>{{ item.specModel }}</td><td>{{ item.selectedManufacturer }}</td><td class="numeric">¥{{ amount(item.selectedPrice) }}</td><td class="numeric">{{ quantity(item.annualTargetQuantity) }}</td><td class="numeric">{{ quantity(item.periodPurchaseQuantity) }}</td><td class="numeric">{{ quantity(item.cumulativePurchaseQuantity) }}</td><td><div class="progress-cell"><span><i :style="{ width: `${Math.min(100, number(item.completionRate))}%` }"></i></span><strong>{{ quantity(item.completionRate) }}%</strong></div></td><td><em class="status-pill">{{ item.selectedFlag }}</em></td><td class="numeric">{{ quantity(item.departmentConsumptionQuantity) }}</td><td :class="{ 'warning-text': item.incompleteReason !== '-' }">{{ item.incompleteReason }}</td></tr></tbody>
        </table>
        <table v-else class="reconciliation-table inventory-summary-table">
          <thead><tr><th>物资编码</th><th>物资名称</th><th>规格</th><th>生产厂家</th><th class="numeric">期初库存</th><th class="numeric">本期入库</th><th class="numeric">本期退库</th><th class="numeric">本期领用</th><th class="numeric">本期消耗</th><th class="numeric">本期报废</th><th class="numeric">期末库存</th><th class="numeric">库存单价</th><th class="numeric">库存金额</th><th>库房</th><th>物资属性</th></tr></thead>
          <tbody><tr v-if="loading"><td colspan="15" class="table-message">正在汇总库存流水…</td></tr><tr v-else-if="!rows.length"><td colspan="15" class="table-message">当前筛选条件下暂无库存流水</td></tr><tr v-for="item in inventoryRows" v-else :key="`${item.warehouseName}-${item.productCode}`"><td class="code-cell">{{ item.productCode }}</td><td class="product-cell">{{ item.productName }}</td><td>{{ item.specModel }}</td><td>{{ item.manufacturerName }}</td><td class="numeric">{{ quantity(item.openingQuantity) }}</td><td class="numeric positive-text">{{ quantity(item.inboundQuantity) }}</td><td class="numeric">{{ quantity(item.returnQuantity) }}</td><td class="numeric">{{ quantity(item.requisitionQuantity) }}</td><td class="numeric">{{ quantity(item.consumptionQuantity) }}</td><td class="numeric danger-text">{{ quantity(item.scrapQuantity) }}</td><td class="numeric strong-number">{{ quantity(item.closingQuantity) }}</td><td class="numeric">¥{{ amount(item.unitPrice) }}</td><td class="numeric strong-number">¥{{ amount(item.inventoryAmount) }}</td><td>{{ item.warehouseName }}</td><td><em class="status-pill neutral">{{ item.materialAttribute }}</em></td></tr></tbody>
        </table>
      </div>
      <footer class="reconciliation-table-footer"><p>{{ config.note }}</p><div class="report-pagination"><span>共{{ total.toLocaleString('zh-CN') }}条</span><select v-model.number="query.size" @change="query.page = 1; loadReport()"><option :value="20">20条/页</option><option :value="50">50条/页</option><option :value="100">100条/页</option></select><button type="button" :disabled="query.page === 1" @click="changePage(query.page - 1)">‹</button><button v-for="page in pageNumbers" :key="page" type="button" :class="{ active: page === query.page }" @click="changePage(page)">{{ page }}</button><button type="button" :disabled="query.page === totalPages" @click="changePage(query.page + 1)">›</button></div></footer>
    </section>
  </main>
</template>
