...
/** Enforces fine-grained quota-package permissions at the business-service boundary. */
@Service
public class QuotaPermissionGuard {
    private final JdbcTemplate jdbcTemplate;
    private final OperatorContextProvider operatorContextProvider;

    public QuotaPermissionGuard(JdbcTemplate jdbcTemplate, OperatorContextProvider operatorContextProvider) {
    ...
    public void require(String permissionCode) {
        ...
        if (operator.roles().stream().anyMatch(role -> "admin".equalsIgnoreCase(role)
                || "ROLE_ADMIN".equalsIgnoreCase(role) || "ROLE_SYSTEM".equalsIgnoreCase(role))) return;
        ...
        if (count == null || count == 0) throw new AccessDeniedException("无权限执行当前定数包操作：" + permissionCode);
    }
}
